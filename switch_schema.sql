...
COMMENT ON SCHEMA switch
  IS 'This schema contains core tables for use with SWITCH-pyomo. All major "data blocks", such as projects, load zones, fuel prices, etc, are indexed by scenario ids. These are used in the scenarios_switch table to set up different SWITCH scenarios that you can run. A script called get_switch_input_tables.py reads those keys and constructs input tables according to the specified scenarios ids.';
...
COMMENT ON TABLE study_timeframe
  IS 'Defines a time frame for one or more studies which includes a set of periods and timeseries within each period. If you wish to change the length or number of investment periods, you will need to define a new row in this table.';
...
COMMENT ON TABLE period
  IS 'Defines investment periods within a given study timeframe. These are block of time in which investment can occur and are typically between 1 and 10 years long. Each period is linked to one or more raw timeseries.';
...
COMMENT ON TABLE period_all_timeseries
  IS 'The raw timeseries that describe operational conditions in this period. For statistical purposes, this is the population of time-based data to sample from. Raw timeseries may be shared across different study timeframes. If all of the raw timeseries are bootstrapped from a single year of historical data, then you would only include one year in a given period (even if the period is several years long) because including more years won''t provide more diversity of renewable outputs.';
...
COMMENT ON TABLE time_sample
  IS 'A representative sample drawn from all available timeseries in this study timeframe. A single study timeframe may have multiple sample drawn from it.';
...
COMMENT ON TABLE sampled_timepoint
  IS 'A set of sampled timepoints, organized into timeseries. Redundant index columns are intentional to allow faster queries.';
...
COMMENT ON TABLE demand_timeseries
  IS 'Hourly demands in MW per load zone. Contains different demand scenarios. Note, the load_zone_name and timestamp_utc are redundant with data in load_zone and raw_timepoint, and are provided for convenience. The foreign key checks ensure these redundant data values match the data in their primary tables.';
...
COMMENT ON TABLE transmission_lines
  IS 'This table contains all transmission lines defined in the simulation, regardless if the transmission line currently exists or is merely being proposed. Transmission lines must only be defined in one direction. Switch will automatically augment the model to allow for Tx in both directions. ';
...
COMMENT ON TABLE energy_source
  IS 'Contains fuels and non-fuel energy sources such as Solar or Wind. Fuels should specify their CO2 intensities, and non-fuel energy sources should leave those columns empty.';
...
COMMENT ON TABLE generation_plant
  IS 'Defines generation and/or storage projects. These may denote individual generating units, a power plant that includes several generating units, or an aggregation of several different plants that can be dispatched together without regard for unit commitment considerations. The columns without NOT NULL constraints are optional, and should only be populated if that field is relevant for a particular generation project.';
...
COMMENT ON TABLE generation_plant_scenario
  IS 'Defines which set of generation projects to include in a particular SWITCH optimization. Use this to enable/disable projects, aggregate projects for faster planning simulations without unit commitment, etc.';
...
COMMENT ON COLUMN variable_capacity_factors.timestamp_utc IS 'This timestamp should be recorded in UTC. For regions that only span one time zone, this is less important. This should never contain daylight savings time because a time series specified in daylight savings time will drop one hour per year.';
...
COMMENT ON TABLE scenario
    IS 'This table defines the simulation parameters for a Switch Pyomo run. The get_switch_input_tables.sh script pulls the chosen data into the tab and dat files the model requires. It parses command line arguments and looks for the "s" option, which specifies the simulations scenario. That id is used as a key to read this table and extract all other ids. All of the id columns reference the tables where the parameters are described. They will update if you update those values and will raise an error when you try to delete an id that is still being used in some scenario in this table.';
